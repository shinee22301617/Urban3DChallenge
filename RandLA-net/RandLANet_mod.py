...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.inputs['features'] = flat_inputs[4 * num_layers] #20
            self.inputs['labels'] = flat_inputs[4 * num_layers + 1] #21
    ...
    def inference(self, inputs, is_training):
        ...
        batch = tf.Variable(self.training_epoch * self.training_step)
        ...
        for i in range(self.config.num_layers):
            tmp = inputs['xyz'][i]
            print(type(inputs['xyz'][i]))
            print(inputs['xyz'][i].get_shape())
            batch_size = tf.shape(inputs['xyz'][i])[0]
            print(batch_size.get_shape())
            index_input = tf.reshape(inputs['neigh_idx'][i], shape=[batch_size, -1])
            print(type(index_input))
            print(index_input.get_shape())
            pred, end_points = get_model(inputs['xyz'][i],index_input, is_training, 13, None, 0)
        '''
        #add mlp to 8
        ...
        for i in range(self.config.num_layers):
            '''end mod'''
            #dilated_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training)
    ...
    #mod
    @staticmethod
    def trilinear_interpolation(feature, interp_idx):
        """
        :param feature: [B, N, d] input features matrix
        :param interp_idx: [B, up_num_points, 1] nearest neighbour index 
        :return: 
...
def get_model(point_cloud, neigh_cloud, is_training, num_class, bn_decay=None, weight_decay=None, feature_channel=0):
#Semantic segmentation PointNet, input is B x N x3 , output B x num_class 
  ...
  print("---------------------------")
  print(neigh_cloud.get_shape())
  print(neigh_cloud.shape)
  ...
  if feature_channel > 0:
    ...
    l0_neigh = neigh_cloud

  end_points['l0_xyz'] = l0_xyz
  num_points = [num_point//8, num_point//32, num_point//128, num_point//256]
  # Feature encoding layers
  l1_xyz, l1_points = PointASNLSetAbstraction(l0_xyz, l0_points, l0_neigh, npoint=num_points[0], nsample=32, mlp=[32,32,64], is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay, scope='layer1', as_neighbor=8)
  l2_xyz, l2_points = PointASNLSetAbstraction(l1_xyz, l1_points, l1_neigh, npoint=num_points[1], nsample=32, mlp=[64,64,128], is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay,scope='layer2', as_neighbor=4)
  l3_xyz, l3_points = PointASNLSetAbstraction(l2_xyz, l2_points, l2_neigh, npoint=num_points[2], nsample=32, mlp=[128,128,256], is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay, scope='layer3', as_neighbor=0)
  l4_xyz, l4_points = PointASNLSetAbstraction(l3_xyz, l3_points, l3_neigh, npoint=num_points[3], nsample=32, mlp=[256,256,512], is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay, scope='layer4', as_neighbor=0)
