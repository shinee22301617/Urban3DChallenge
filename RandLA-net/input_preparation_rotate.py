...
if __name__ == '__main__':
    ...
    for sample_type in preparation_types:
        for pc_path in files:
            ...
            if exists(join(out_folder, cloud_name + '_rotate_KDTree.pkl')):
            ...
            rotation_matrix = np.array([[cosval, 0, sinval],
                                        [0, 1, 0],
                                        [-sinval, 0, cosval]])
            rotate_xyz = np.float32(xyz.dot(rotation_matrix))
            #######  Rotate  #########

            sub_ply_file = join(out_folder, cloud_name + '_rotate.ply')

            if sample_type == 'grid':
                sub_xyz, sub_rgb, sub_labels = DP.grid_sub_sampling(rotate_xyz, rgb, labels, grid_size)
            else:
                sub_xyz, sub_rgb, sub_labels = DP.random_sub_sampling(rotate_xyz, rgb, labels, random_sample_ratio)

            sub_rgb = sub_rgb / 255.0
            sub_labels = np.squeeze(sub_labels)
            write_ply(sub_ply_file, [sub_xyz, sub_rgb, sub_labels], ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])
            search_tree = KDTree(sub_xyz, leaf_size=50)
            kd_tree_file = join(out_folder, cloud_name + '_rotate_KDTree.pkl')
            with open(kd_tree_file, 'wb') as f:
                pickle.dump(search_tree, f)
            print("KDTREE COMPLET")

            proj_idx = np.squeeze(search_tree.query(rotate_xyz, return_distance=False))
            proj_idx = proj_idx.astype(np.int32)
            proj_save = join(out_folder, cloud_name + '_rotate_proj.pkl')
