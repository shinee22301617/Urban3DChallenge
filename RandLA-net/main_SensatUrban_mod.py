    ...
    def get_batch_gen(self, split):
        ...
        def spatially_regular_gen():
            ...
            for i in range(num_per_epoch):  # num_per_epoch
                ...
                if len(points) < cfg.num_points:  #使用kdtree文件搜索中心点周围最近的pick_point(65536)个点的序号
    ...
    def get_tf_mapping2(self):
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            ...
            for i in range(cfg.num_layers):
                ...
                #sub_features = batch_features[:, :tf.shape(batch_features)[1] // cfg.sub_sampling_ratio[i], :]
                pool_i = neighbour_idx[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                #下採樣後的每個點在沒有下採樣前的最近id，在上採樣使用
                up_i = tf.py_func(DP.knn_search, [sub_points, batch_xyz, 1], tf.int32)
                input_points.append(batch_xyz)
                #input_features.append(sub_features)
                input_neighbors.append(neighbour_idx)
                input_pools.append(pool_i)
                input_up_samples.append(up_i)
                batch_xyz = sub_points
                
            input_list = input_points + input_neighbors + input_pools + input_up_samples + input_features
