...
def sampling(npoint, pts, nei, feature=None):
    '''
    inputs:
...
def PointASNLSetAbstraction(xyz, feature, neigh, npoint, nsample, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_knn=True, radius=None, as_neighbor=8, NL=True):
    ''' Input:
    ...
    with tf.variable_scope(scope) as sc:
        ...
        print(tf.shape(neigh)[0])
        print(neigh.get_shape())
        
        if num_points == npoint:
            new_xyz = xyz
            new_feature = feature
        else:
            new_xyz, new_feature = sampling(npoint, xyz, neigh, feature)
